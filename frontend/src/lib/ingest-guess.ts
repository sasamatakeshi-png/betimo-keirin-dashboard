...
export const NAMING_RULES = {
  ...
  traffic:
    "推奨: 6月流入経路.csv / 6月外部流入.csv / 6月関連動画.csv / 6月YouTube検索.csv。種別はファイル名から自動判別、対象月はファイル名の「N月」で当年の月を自動選択（年は対象月セレクタで確定）。",
  x: "推奨: 6月X.csv（例）。日別行(1行=1日)を date 一意で取り込み、同じ日付は最新値で置換します。対象月の指定は不要（日付はCSVの Date 列から）。",
} as const;

// 流入経路系CSVの種別を推測。「検索」「関連動画」「外部」「流入経路」で判別。判別不能は null。
export function guessTrafficKind(name: string): TrafficSourceKind | null {
  if (/検索|search/i.test(name)) return "search_term";
